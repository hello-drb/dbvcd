...
function ServiceCard({ title, description, number, isLeading = false }: ServiceProps) {
  return (
    <motion.div
      ...
      className={`group relative border border-[var(--border)] rounded-lg p-8 sm:p-10 backdrop-blur-sm transition-all duration-300 warm-glow ${
        isLeading ? "col-span-1 md:col-span-2 lg:col-span-3 bg-gradient-to-r from-[var(--bg-card)] to-[var(--bg-elevated)]" : "bg-[var(--bg-card)]"
      }`}
    >
      <div className="absolute inset-0 bg-gradient-to-r from-[var(--accent-gold)]/0 via-[var(--accent-gold)]/0 to-[var(--accent-gold)]/0 group-hover:from-[var(--accent-gold)]/10 group-hover:via-[var(--accent-gold)]/5 group-hover:to-[var(--accent-gold)]/10 rounded-lg transition-all duration-500" />

      <div className="relative z-10">
        <div className="flex items-start justify-between mb-5">
          <motion.h3
            className={`font-serif font-light tracking-tight text-[var(--text-primary)] ${
              isLeading ? "text-3xl sm:text-4xl" : "text-2xl"
            }`}
          >
            {title}
          </motion.h3>
          <span className="text-[var(--accent-gold)] text-sm font-light tracking-widest font-sans">
            {number}
          </span>
        </div>
...
export default function Services() {
  ...
  return (
    <section id="services" className="relative w-full py-28 sm:py-40 px-6 sm:px-8 bg-[var(--bg-primary)]">
